...
from homeassistant.const import (CONF_DISPLAY_OPTIONS, CONF_TYPE, CONF_SCAN_INTERVAL, CONF_USERNAME)
...
class evnhanoiclass(Entity):
    ...
    def __init__(self, sensor_type):
        ...
        self._description = 'Version 1.0 - 27-04-2020'
        self.update()
    ...
    def update(self):
        ...
        data_ncov = getdataevn.getlastestmonth(customerId, datetime.datetime.now().year)
        if self.type == 'customerId':
            self._state = data_ncov['customerId']
        elif self.type == 'month':
            self._state = data_ncov['month']
        elif self.type == 'year':
            self._state = data_ncov['year']
        elif self.type == 'power':
            self._state = data_ncov['power']
        elif self.type == 'totalMoney':
            self._state = data_ncov['totalMoney']